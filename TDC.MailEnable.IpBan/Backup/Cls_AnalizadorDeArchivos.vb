...
Namespace Backup
    Public Class Cls_AnalizadorDeArchivos
            ...
            If Not IO.File.Exists(OriginalPath(Archivo)) Then
                ...
                Try
                    If IO.Path.GetExtension(Archivo).ToUpper = $".{NameOf(MAI)}" Then
                        ...
                        Dim Fecha As Date
                        If Cleaner.Contains(Cleaner.GetColString(Core.BDD.MailBackupCleaner.Columnas.Archivo), Archivo) Then
                            Dim Item As Integer = Cleaner.GetItemIndex(Cleaner.GetColString(Core.BDD.MailBackupCleaner.Columnas.Archivo), Archivo)
                        ...
                        If Not Antiguedad > Configuracion.ANTIGUEDAD_EMAILS AndAlso Not MAI.Contains(Core.BDD.MailBackup_MAI.Columnas.Archivo.ToString, Archivo) Then Indexar = True
                        ...
                        If Eliminar AndAlso MAI.Contains(Core.BDD.MailBackup_MAI.Columnas.Archivo.ToString, Archivo) Then Limpiar = True
        ...
        Private Sub BuscadoresEvents_Background(Sender As Object, Detener As Core.Bucle.BackgroundEventArgs) Handles BuscadoresEvents.BackGround
            If Not IsNothing(Archivos) Then
                ...
                Else
                    'Detener = True
                    If BuscadorArchivos.Estado = Cls_BuscadorDeArchivos.EnumEstado.Analizado Then
        ...
        Private Sub BuscadoresEvents_Foreground(Sender As Object, Detener As Core.Bucle.BackgroundEventArgs) Handles BuscadoresEvents.ForeGround
            ...
            If Buscadores.Count > Configuracion.ANALIZADORES_BACKUP AndAlso Not Archivos.IsEmpty Then
                'Extraer Buscadores
                Dim Buscador As Core.Bucle.DoBucle = Nothing
                If Buscadores.ContainsKey(CType(Sender, Core.Bucle.DoBucle).Name) Then
        ...
        Private Sub BuscadoresEvents_Endground(Sender As Object, Detener As Core.Bucle.BackgroundEventArgs) Handles BuscadoresEvents.EndGround
            If Not IsNothing(Archivos) Then
                If Not Archivos.IsEmpty Then CType(Sender, Core.Bucle.DoBucle).Iniciar()
